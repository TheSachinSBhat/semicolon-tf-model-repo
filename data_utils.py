...
def basic_tokenizer(sentence):
  ...
  for space_separated_fragment in sentence.strip().split():
    splitVar = space_separated_fragment
    splitDone = re.split(_WORD_SPLIT, splitVar)
    words.extend( splitDone )
...
def sentence_to_token_ids(sentence, vocabulary, tokenizer=None, normalize_digits=True):
  ...
  #bp()
  #---removed b#return [vocabulary.get(re.sub(_DIGIT_RE, b"0", w), UNK_ID) for w in words]
...
def prepare_custom_data(working_directory, train_enc, train_dec, test_enc, test_dec, enc_vocabulary_size, dec_vocabulary_size, tokenizer=None):
    ...
    enc_vocab_path = os.path.join(working_directory, "vocab%dENC.txt" % enc_vocabulary_size)
    dec_vocab_path = os.path.join(working_directory, "vocab%dDEC.txt" % dec_vocabulary_size)
